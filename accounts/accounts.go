...
// Package accounts implements high level morchain account management.
package accounts
...
// Account represents an morchain account located at a specific location defined
// by the optional URL field.
type Account struct {
	Address common.Address `json:"address"` // morchain account address derived from the key
...
type Wallet interface {
	...
	// chain state reader.
	SelfDerive(bases []DerivationPath, chain morchain.ChainStateReader)
...
//
//	keccak256("\x19morchain Signed Message:\n"${message length}${message}).
//
...
func TextAndHash(data []byte) ([]byte, string) {
	msg := fmt.Sprintf("\x19morchain Signed Message:\n%d%s", len(data), string(data))
